...
setup() {
	$SHELL _tutr/screen_size.sh 40 80
	export _HERE=$PWD
	...
	[0m  /XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\\
	...
	[0m        _______..______      ___       ______  _______  _______  
	       /       ||   _  \\    /   \\     /      ||   ____||       \\ 
	      |   (----\`|  |_)  |  /  ^  \\   |  ,----'|  |__   |  .--.  |
	       \\   \\    |   ___/  /  /_\\  \\  |  |     |   __|  |  |  |  |
	TEXT
	...
	[0m                  AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
_tutr_check_shortcuts() {
	# Should I achieve this with if [[ -n $BASH/$ZSH_NAME ]] ?
	if expr $SHELL : '.*zsh' >/dev/null; then
		...
		_tutr_die "Unknown shell $SHELL: Unable to install 'shortcuts' command in this shell"
	fi
}
...
_tutr_install_shortcuts() {
	...
	REPLY=
	while true; do
		echo -en $'\n\x1b[1;33mTutor\x1b[0m| May I make this one-time change to' $1? "[Y/n] "
			...
			[Yy])
				...
				shortcuts() { # DO NOT MODIFY. SHORTCUTS COMMAND ADDED BY SHELL TUTORIAL
					...
					---------|-----------------------------------------------
					  ...
					  Right  | Move cursor FOREWARD one character
					Backspace| Delete the character to the LEFT of the cursor
					  Delete | Delete the character on the RIGHT the cursor
					  ^A     | Move cursor to BEGINNING of line
					  ^E     | Move cursor to END of line
					  M-B    | Move cursor BACKWARD one whole word
					  M-F    | Move cursor FOREWARD one whole word
					  ...
					  M-D    | Cut FORWARD from cursor to end of word
				...
				;;
			[Nn])
				_tutr_info echo "Ok, I will leave your shell\'s startup file unchanged."
				break
				;;
		esac
	done
...
echo_1_test() {
	...
	_tutr_generic_test -c echo -l 2 -d "'$_BASE'" -a disorganized.txt
}
...
echo_2_test() {
	_tutr_nonce && return $PASS
	_tutr_generic_test -c echo -l 2 -d "'$_BASE'" -a disorganized.txt -a combinatorially.txt
}
...
echo_2_epilogue() {
	...
	through, and you don't even in what order they go.  If this is what
...
history_echo_cat_test() {
	...
	_tutr_generic_test -c cat -l 2 -d "'$_BASE'" -a disorganized.txt -a combinatorially.txt
}
...
history_echo_cat_epilogue() {
	...
	on the keyboard to contol the cursor.  This is due, in part, to the fact
	that mice weren't widespred when terminals were developed.
...
ctrl_a_test() {
	_tutr_nonce shortcuts && return $PASS
	_tutr_generic_test -c cat -l 2 -d "'$_BASE'" -a reorganization.txt -a disorganized.txt -a combinatorially.txt
}
...
ctrl_a_epilogue() {
	...
	Why does the shell call the 'Alt' key 'Meta' when your keyboard doesn't
	have any such key?

	The keyboard you're using now either follows the layout of the IBM
	Personal Computer of 1981 or is an Apple keyboard from 1983 (the two
	most common keyboard layouts in the 21st century).  The mainframes that
	the Unix command shell was created for on weren't made by IBM, nor were
	they "personal" computers.  The keys surrounding their spacebar were
...
tab_prologue() {
	...
	21st century innovations are great timesavers.  Oh, did I say 21st
...
tab_test() {
	_tutr_nonce shortcuts && return $PASS
	_tutr_generic_test -c cat -l 2 -d "'$_BASE'" -a sequentializing.txt -a obfuscatory.txt -a reorganization.txt -a disorganized.txt -a combinatorially.txt
}
...
cutpaste_words_test() {
	_tutr_nonce shortcuts && return $PASS
	_tutr_generic_test -c cat -l 2 -d "'$_BASE'" -a reorganization.txt -a disorganized.txt -a combinatorially.txt -a sequentializing.txt -a obfuscatory.txt
}
...
cut_line_prologue() {
	...
	if [[ $_SH = Zsh ]]; then
		cat <<-:
		^U cuts the entire command line into the yank buffer REGARDLESS of where
		the cursor is.  You can then run as many commands as you want before
		restoring the original command from the yank buffer with ^Y.
		...
		cat <<-:
		^U cuts the from the position of the cursor BACK to the beginning of
		the command line, storing the cut text into yank buffer.  You can
		then run as many commands as you want before restoring the original
...
cut_line_test() {
	_tutr_generic_test -c ls -l 1 -d "'$_BASE'"
}
...
finish_prologue() {
	...
	like a pro.  This was a lot of new knowledege to throw on you all at
...
finish_test() {
	_tutr_nonce shortcuts && return $PASS
	_tutr_generic_test -c cat -l 2 -d "'$_BASE'" -a painstakingly.txt -a nonsequential.txt -a fragmentary.txt -a reorganization.txt -a disorganized.txt -a combinatorially.txt -a sequentializing.txt -a obfuscatory.txt
}
