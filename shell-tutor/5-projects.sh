...
# * Learn the dir structure of assignments
# * Combine the skills you now have to put files into their proper places
...
setup() {
	...
	Do you remember the "^" and "M-" mnemomics you learned in back in the
...
mkdirs_test() {
	...
	else _tutr_generic_test -c mkdir -l 2 -d "'$_BASE'"
	fi
}
...
sort_files_test() {
	...
	else _tutr_generic_test -c sort_files -l 1
	fi
}
...
cd_into_src_test() {
	...
	else _tutr_generic_test -c cd -a src -l 1 -d "'$_BASE/src'"
	fi
}
...
run_tests_pre() {
	export Python_Ver=$(python -V 2>&1)
	export P3V=$(python3 -V 2>&1)
	if [[ $Python_Ver = "Python 2"* && $P3V = "Python 3"* ]]; then
		export PY=python3
	else
		export PY=python
	fi
...
run_tests_prologue() {
	# If Python_Ver < 3, recommend running 'python3'
	if [[ $Python_Ver != "Python 3"* && $P3V != "Python 3"* ]]; then
		# Neither Python nor Python3 are valid
		cat <<-:
		It appears that you do not have a valid installation of Python.
		Neither 'python' nor 'python3' invoke Python version 3.
		Please exit the lesson and install Python 3 to complete this lesson.
	...
	Python programs can be run on the command line by invoking the '$PY'
	...
	'runTests.py' with the '$PY' command.
...
run_tests_test() {
	...
	elif [[ ${_CMD[@]} = $PY ]]; then return 99
	elif _tutr_lev "${_CMD[0]}" $PY 3; then return $MISSPELD_CMD
	elif [[ $PWD != "$_BASE/src" ]]; then return $WRONG_PWD
	elif [[ ${_CMD[@]} = "$PY runTests.py" || ${_CMD[@]} = "python3 runTests.py" ]]; then return 0
...
nano_main_py_prologue() {
	...
	Resist the urge to fix the error and leave 'main.py' unchagned.
	This is just a recon mission.
	:
}

nano_main_py_test() {
	_tutr_generic_test -c nano -a main.py -l 2 -d "'$_BASE/src'"
}
...
edit_plan_test() {
	...
	if   (( $_CHANGED == $_T )); then return 97
	else _tutr_generic_test -c nano -a Plan.md -l 2 -d "'$_BASE/doc'"
	fi
}
...
fix_bug_prologue() {
	...
	Re-run the tests with '$PY runTests.py'.
...
fix_bug_test() {
	...
	elif [[ ${_CMD[@]} == "$PY runTests.py" && $_RES != 0 ]]; then return 99
	else _tutr_generic_test -c $PY -a runTests.py -l 2 -d "'$_BASE/src'"
	fi
}
...
fix_bug_hint() {
	case $1 in
		...
		*)
			_tutr_generic_hint $1 $PY "$_BASE/src"
...
signature_test() {
	...
	else _tutr_generic_test -c nano -a Signature.md -l 2 -d "'$_BASE/doc'"
	fi
}
