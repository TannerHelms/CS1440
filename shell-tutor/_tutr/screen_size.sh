...
if [[ $# -lt 2 ]]; then
    echo Usage: _tutr_screen_size.sh ROWS COLS
    exit 1
fi
...
_tutr_check_scrsz() {
    ...
    if [[ -n $NEED ]]; then
        ...
        tput cup 1 1
        echo -n "Your terminal is ${L} rows tall x ${C} columns wide"
        tput cup 2 1
        echo -n "Resize it until it is at least ${TARGET_L}x${TARGET_C}"
        tput cup 3 1
