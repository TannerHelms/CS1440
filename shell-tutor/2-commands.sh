# Lesson #2
#
# * Use the 'which' command to find where a program is installed
# * Use the 'type' command to reveal the type of a command
# * Some commands have built-in help accessible with --help or -h
# * Use the 'man' command to read the system manual
# * Learn why some programs must be run by typing './' in front of their name
...
which_ls_prologue() {
	...
	Each time you enter a command $_SH searches for a program with a matching
	name and runs it.  If that search comes up empty you will see a "command
	not found" error.  You can learn where $_SH will find 'ls' on your computer
	by using the 'which' command.

	'which' takes the name of a program as an argument.  It either prints the
	full path to the program OR a message saying that program could not be
	found.  Programmers use 'which' to test whether a program is installed or
	to see which version $_SH will run.
...
which_ls_test() {
	_tutr_generic_test -c which -l 2 -a ls
}
...
which_ls_hint() {
	...
	Use 'which' to find out where your 'ls' command resides on your computer.
	:
}
...
which_ls_epilogue() {
	...
	The output of 'which' shows you exactly where you can find the 'ls' program
	on your computer.  The "${_LS%/ls}" part of the output names the directory
	(or directories) that contain the 'ls' program.

	The string "$_LS" is called the "path" to the 'ls' program.  In this case
	the path starts with a front-slash '/' character, which instructs the shell
	to start its search at the very beginning of the directory structure of the
	computer.
...
which_cat_prologue() {
	cat <<-:
	Use 'which' to find out where the 'cat' command resides on your computer.
	:
}

which_cat_test() {
	_tutr_generic_test -c which -l 2 -a cat
}
...
which_cat_hint() {
	...
	Use 'which' to find out where your 'cat' command resides on your computer.
	:
}
...
which_which_test() {
	_tutr_generic_test -c which -l 2 -a which
}
...
which_which_epilogue() {
	...
	if [[ ${_LS%/ls} == ${_CAT%/cat} && ${_LS%/ls} == ${_WHICH%/which} ]]; then
		cat <<-:
	...
	All along you have been running thes commands without telling $_SH their
	absolute paths.  How did $_SH know where to find them?
...
echo_path_test() {
	_tutr_generic_test -c echo -l 2 -a '\$PATH'
}
	...
	case $1 in
		$WRONG_ARGS)
			if ! expr index ${_CMD[1]} '$' >/dev/null; then
				...
				echo "Capitalize the entire word '\$PATH'"
			else
...
which_cd_test() {
	_tutr_generic_test -c which -l 2 -a cd
}
...
which_cd_epilogue() {
	...
	Depending upon which shell you're using, you either saw a message that
	told you that the 'cd' program was not found or that 'cd' is a shell
	built-in.

	Some commands that you run are separate programs from the shell.  These
	are called "external" commands.  'ls', 'cat' and 'which' are external
	commands.  External commands are installed somewhere on your hard drive.
	...
	Most of the time the distinction between "external" and "internal"
	doesn't matter: just enter commands and trust the shell to run them.
	But when something doesn't work as expected it's helpful to understand
	why.
	...
	Your own programs are just as "real" to the shell as any other command.
	Later on in the CS1440 course you will create your own command line
	tools and learn how to "install" them this way.
...
type_cd_prologue() {
	...
	that are built in to $_SH.  You need a built-in command to detect
	another built-in.

	'type' is the built-in command that can do this.  'type' works just like
	'which', by taking the name of a command as its argument.  'type' can
	tell you whether a command is
...
type_cd_test() {
	_tutr_generic_test -c type -l 2 -a cd
}
...
type_ls_test() {
	_tutr_generic_test -c type -l 2 -a ls
}
...
type_cat_test() {
	_tutr_generic_test -c type -l 2 -a cat
}
...
# XXX Cut these out - this is too fraught with trouble
cat_help_prologue() {
	cat <<-:
...
cd_help_test() {
	_tutr_generic_test -c cd -l 1 -a --help
}
...
man_ls_prologue() {
	...
	*   The command shell long predates the world-wide-web, and moreso
...
man_ls_test() {
	_tutr_generic_test -c man -l 2 -a ls
}
...
man_cat_test() {
	_tutr_generic_test -c man -l 2 -a cat
}
...
man_cd_test() {
	_tutr_generic_test -c man -l 2 -a cd
}
...
man_man_prologue() {
	...
	need help with.  But what can you do when you don't know what are
	looking for?
...
man_man_test() {
	_tutr_generic_test -c man -l 2 -a man
}
...
man_k_test() {
	...
	_tutr_generic_test -c man -l 2 -a -k -a manual
}
...
man_k_epilogue() {
	...
	As I said before, one big advantage this has over Google is that 'man
	-k' KNOWS which commands are are installed on this computer.  That's
	something that even Google doesn't know about you (probably).
