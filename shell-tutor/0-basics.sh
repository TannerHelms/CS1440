...
tutor_prologue() {
	...
	are ${#_SKILLS[@]} things to in this lesson.  As you do things the little
	meter down by your prompt turns from red to green.  After you have done
	enough things you'll win, which improves your self-esteem.
...
tutor_test() {
	[[ ${_CMD[@]} == 'tutor hint' ]] && return 0
	[[ ${_CMD[0]} == tutor && ${#_CMD[@]} -gt 2 ]] && return $TOO_MANY_ARGS
	[[ ${_CMD[0]} == tutor && ${#_CMD[@]} -lt 2 ]] && return $TOO_FEW_ARGS
	[[ ${_CMD[0]} == tutor && ${_CMD[1]} != hint ]] &&  return $WRONG_ARGS
	_tutr_lev "${_CMD[0]}" tutor 2 && return $MISSPELD_CMD
	return $WRONG_CMD
}

tutor_hint() {
	case $1 in
		$TOO_FEW_ARGS|$TOO_MANY_ARGS)
			cat <<-:
			Re-display this step's hint by running 'tutor hint'
			:
			;;
		$WRONG_ARGS)
			echo "Run 'tutor hint' (without the quote marks) to proceed."
			;;
		*)
...
tutor_epilogue() {
	...
	As you can see, 'tutor hint' prints your marching orders again.
	This will come in handy.

	By the way, you can quit this tutorial at any time by running 'exit' or
	'tutor quit'.  But the only way to complete it is to work your way
	through to the end.
...
hello_world_prologue() {
	...
	written down in programs.  Consessions were made so that it is quick and
	easy to type:
...
hello_world_epilogue() {
	...
	if [[ "echo Hello, World" != ${_CMD[@]} ]]; then
		...
		matter.  Try to be dilligent at following instructions exactly!
	...
	strings "$_PREV1" and "$_PREV2" were arguments to the 'echo' command
	you just ran.
	...
	In the shell EVERYTHING you type is understood to be a string.  This is
	why you didn't need to put quote marks around the words "$_PREV1" and
	"$_PREV2".

	The comma following "Hello," in the command
	  'echo Hello, World'
	isn't special; it is actually a part of the word "Hello,".

	Now, there will be cases where quote marks are mandatory; this just
	isn't one of them.  Of course, you can use quote marks if you really
	want to:
	  'echo "Hello, World"'
...
echo_no_args_prologue() {
	...
	like 'print()', 'echo' can take any number of arguments.  Zero, one,
	two, a hundred; it's all good.
...
echo_no_args_test() {
	[[ ${_CMD[0]} == echo && ${#_CMD[@]} -ne 1 ]] && return $TOO_MANY_ARGS
	[[ ${_CMD[@]} == echo ]] && return 0
	_tutr_lev "${_CMD[0]}" echo 2 && return $MISSPELD_CMD
	return $WRONG_CMD
}
...
ls_epilogue() {
	...
	if [[ ${_CMD[@]} == dir ]]; then
		...
		I think that extra letter is just too much for lazy Unixfolk to type.
...
ls_a_prologue() {
	...
	four files here.  It turns out that there is a stowaway hidden from
	view.
...
ls_a_test() {
	[[ ${_CMD[0]} == ls && ${#_CMD[@]} -lt 2 ]] && return $TOO_FEW_ARGS
	[[ ${_CMD[0]} == ls && ${#_CMD[@]} -gt 2 ]] && return $TOO_MANY_ARGS
	[[ ${_CMD[@]} == "ls -a" ||  ${_CMD[@]} == "dir -a" ]] && return 0
	[[ ${_CMD[1]} != "-a" ]]  && return $WRONG_ARGS
	[[ ${_CMD[0]} == ls-a ]] && return $NEED_SPACES
	_tutr_lev "${_CMD[0]}" ls 2 && return $MISSPELD_CMD
	[[ ${_CMD[0]} != ls ]] && return $WRONG_CMD
	[[ $PWD != "$_BASE" ]] && return $WRONG_PWD
}
...
reset_prologue() {
	...
	*	'clear' erases everything on the screen
	...
	Now you'll leave the happy path and learn what happens when things go wrong.

	In Unix you can use 'cat' on ANY kind of file you want.  Nothing is stopping
	you from cat-ing an MP3, a ZIP file, a PDF or a JPEG to the screen.  It is
	fun and you may learn something about those files that you wouldn't
	otherwise see.

	However, there is a posibility that doing this will make your terminal go
	haywire.  Don't worry, it's nothing permanent and it won't hurt your
	computer.  It is something that can happen at any time when you're working
	with data.  Before you try it I want to show you how to fix a corrupted
	terminal.
...
plain_cat_prologue() {
	...
	Did you know that you can get 'cat' stuck simply by calling it without
...
plain_cat_epilogue() {
	...
	'Ctrl-C' as "Cancel" predates the familar "Undo", "Cut", "Copy", "Paste"
	shortcuts by a good decade or so.  None of the shortcuts that you have
	used in other applications do what you expect at the terminal.  You'll
	just have to retrain your fingers.
...
cat_nofile_prologue() {
	...
	Get in the habit of carefully reading all messges presented to you.
...
cat_permission_denied_epilogue() {
	...
	if [[ $_RES -eq 0 ]]; then
		...
		Rest assured that I'm just as dissapointed about this as you are.
...
command_not_found_test() {
	if   (( _RES == 0 )); then return $WRONG_CMD
