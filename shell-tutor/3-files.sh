...
# * Remove files
# * Refer to many files at once with wildcards
...
copy01_test() {
	...
	else _tutr_generic_test -l 1 -c cp -a copy0.txt -a copy1.txt -d "'$_BASE'"
	fi
}
...
copy02_test() {
	...
	else _tutr_generic_test -c cp -l 1 -a copy0.txt -a copy2.txt -d "'$_BASE'"
	fi
}
...
cp_clobber_test() {
	_tutr_generic_test -c cp -l 1 -a copy0.txt -a different.txt -d "'$_BASE'"
}
...
cp_clobber_epilogue() {
	...
	any string that begins with "Y" or "y".  These strings are are all
	interpreted as affirmative responses:
...
move01_test() {
	...
	_tutr_generic_test -c mv -l 1 -a move0.txt -a move1.txt -d "'$_BASE'"
}
...
move02_err_test() {
	_tutr_generic_test -c mv -l 1 -a move0.txt -a move2.txt -d "'$_BASE'"
}
...
move12_test() {
	...
	_tutr_generic_test -c mv -l 1 -a move1.txt -a move2.txt -d "'$_BASE'"
}
...
mv_clobber_test() {
	...
	_tutr_generic_test -c mv -l 1 -a move2.txt -a different.txt -d "'$_BASE'"
}
...
rm_1file_test() {
	...
	else _tutr_generic_test -c rm -l 1 -a copy0.txt -d "'$_BASE'"
	fi
}
...
rm_star0_test() {
	...
	else _tutr_generic_test -c rm -l 1 -a *.txt -d "'$_BASE'"
	fi
}
...
rm_star1_test() {
	if   _tutr_nonce; then return $PASS
		...
		else _tutr_generic_test -c rm -l 1 -a *.wav -d "'$_BASE'"
		fi
...
# TODO: what if rm doesn't ask for confirmation?
rm_star2_prologue() {
	cat <<-:
	...
	BE VERY CAREFUL WITH '-f'!!!

	Use 'rm -f' sparingly.  Every Unix hacker their own harrowing story
	about this command.  Don't become another statistic!  Think before you
	type, and think again before you press enter.  Check yourself before you
	wreck yourself.

	If you do find yourself answering hundreds of prompts, cancel the
	command by pressing '^C'.  Be aware that any files that you answered
	"yes" are still deleted.
...
rm_star2_test() {
	...
	else _tutr_generic_test -c rm -l 1 -a *.png -a *.mp4 -d "'$_BASE'"
	fi
}
...
rm_star3_test() {
	...
	else _tutr_generic_test -l 1 -c rm -a -f -a * -d "'$_BASE'"
	fi
}
